...
export async function action({ request }: DataFunctionArgs) {
	...
	invariantResponse(
		formData.get('intent') === 'delete-carModelsGallery',
		'Invalid intent',
	)
	const facilityId = formData.get('facilityId')
	invariantResponse(typeof facilityId === 'string', 'Invalid facilityId')

	const deletedCarModelsGallery = await prisma.carModelsGallery.delete({
		select: { name: true },
		where: {
			id: facilityId,
		},
	...
	const toastHeaders = await createToastHeaders({
		title: 'Deleted!',
		description: `The carModelsGallery "${deletedCarModelsGallery.name}" deleted. 🫡`,
	})
	return json({ status: 'success' } as const, { headers: toastHeaders })
}

export default function CarModelsGalleryDeletion() {
		...
		<>
			...
			<div className={destructiveModalWrapperClassList}>
				<div className="flex flex-col text-center items-center">
					<p className="text-md">Deleting CarModelsGallery</p>
					<div className="my-6 text-sm flex justify-center gap-3 bg-backgroundDashboard py-2 px-4 w-fit rounded-lg">
						<p>{data.carModelsGallery.name}</p>
					</div>

					<deleteFetcher.Form method="POST" className="flex justify-center">
						<input name="facilityId" value={data.carModelsGallery.id} type="hidden" />
						<StatusButton
							name="intent"
							value="delete-carModelsGallery"
							variant="destructive"
							size="sm"
							status={
