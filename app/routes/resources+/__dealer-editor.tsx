...
export function DealerEditor({
	...
	dealer?: SerializeFrom<
		Pick<Dealer, 'id' | 'url' | 'name' | 'state' | 'city' | 'address'> & {
			images: Array<Pick<DealerImage, 'id' | 'altText'>>
		}
	>
}) {
	const dealerFetcher = useFetcher<typeof action>()
				...
				*/}
				...
				<div className="flex flex-col gap-1">
					<div className="flex gap-5">
						<Field
							labelProps={{ children: `dealer's URL` }}
							inputProps={{
								autoFocus: true,
						...
						/>
						<Field
							labelProps={{ children: 'Dealer Name' }}
							inputProps={{
					...
					<div className="flex gap-5">
						<SelectBox
							...
							inputProps={{
								...conform.input(fields.state, { ariaAttributes: true }),
							}}
							errors={fields.state.errors}
							options={countryList}
							defaultOption={dealer?.state ?? ''}
							className="w-1/2"
						/>
					...
					<Field
						labelProps={{ children: 'Address (street, building, etc.)' }}
						inputProps={{
...
export function ErrorBoundary() {
	return (
		<GeneralErrorBoundary
			statusHandlers={{
				404: ({ params }) => (
					<p>No dealer with the id "{params.dealerId}" exists</p>
				),
			}}
		/>
	)
